...
int main(void)
{
    ...
    int TOF_data[1];        // array of TOF data, TOF 0x29 first 0x2C last
    ...
    while (1)
    {
	    ...
	    if ((TOF_data[0] < 1000) && (TOF_data[1] < 1000)) {
        ...
        case (ACCELERATE): // since the drive by wire doesn't respond to the car, the function should send back the speed it's trying to set the car too
            ...
            //    car.state = TURN_LEFT;
            //}
            else if ((TOF_data[0] > 1000) && (TOF_data[1] > 1000))
            {
		printf("TRANSITIONING TO IDLE\n");
                printf("TRANSITIONING TO TURN_LEFT\n");
                car.state = IDLE;
        ...
        case (TURN_RIGHT):
            ...
            {
                printf("TRANSITIONING TO ACCELERATE    try:n");
                car.state = ACCELERATE;
            }
